...
>(({ className, ...props }, ref) => (
  ...
  >
    <SwitchPrimitives.Thumb
      className={cn(
        'pointer-events-none relative inline-flex items-center justify-center h-5 w-5 rounded-full bg-background shadow-lg ring-0 transition-all data-[state=checked]:translate-x-6 data-[state=unchecked]:translate-x-0',
      )}
    >
      <div className="flex items-center justify-center w-full h-full">
        {props.checked ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />}
      </div>
    </SwitchPrimitives.Thumb>
  </SwitchPrimitives.Root>
));
